...
<body>
	...
	<div class="container py-4">
		...
		<div class="row g-4">
			...
			<div class="col-lg-5">
				<!-- Preview Dokumen -->
				@if(session('last_document_path'))
				<div class="card shadow-sm mb-3">
					<div class="card-header bg-white fw-bold">Dokumen Upload</div>
					<div class="card-body text-center p-2 bg-light rounded-bottom">
						@php
							$ext = pathinfo(session('last_document_path'), PATHINFO_EXTENSION);
							$isImage = in_array(strtolower($ext), ['jpg','jpeg','png','webp','gif']);
							$docUrl = asset('storage/' . session('last_document_path'));
						@endphp
						@if($isImage)
							<a href="{{ $docUrl }}" target="_blank">
								<img src="{{ $docUrl }}" class="img-fluid rounded" style="max-height: 300px; object-fit: contain;">
							</a>
						...
						@else
							<div class="py-4">
								...
								<p class="mt-2 mb-2 fw-bold">{{ session('last_document_original_name') ?? 'Dokumen PDF' }}</p>
								<a href="{{ $docUrl }}" target="_blank" class="btn btn-sm btn-primary">Lihat PDF</a>
							</div>
						@endif
					</div>
				</div>
