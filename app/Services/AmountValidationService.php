...
class AmountValidationService
{
    private array $terbilangMap = [
        ...
        'ratus' => 100, 'ribu' => 1000, 'juta' => 1000000,
    ];
    ...
    private function validasiTotal(array $data): array
    {
        ...
        if (!$total) {
            return ['status' => 'no_data', 'message' => 'Total pembayaran tidak terdeteksi', 'required' => true];
        }

        if (empty($items)) {
            return ['status' => 'info', 'message' => 'Total: Rp ' . number_format($total, 0, ',', '.'), 'required' => true];
        }
    ...
    private function validasiTerbilang(array $data): array
    {
        ...
        if (!$terbilang) {
            return ['status' => 'no_data', 'message' => 'Terbilang tidak ditemukan', 'required' => true];
        }

        if (!$total) {
            return ['status' => 'no_data', 'message' => 'Total tidak tersedia', 'required' => true];
        }

        $nilaiTerbilang = $this->parseTerbilang($terbilang);

        if (!$nilaiTerbilang) {
            return ['status' => 'warning', 'message' => 'Gagal membaca terbilang', 'required' => true];
        }
    ...
    private function validasiPPN(array $data): array
    {
        ...
        if (!$total || !$ppn) {
            return ['status' => 'no_data', 'message' => 'Data PPN tidak lengkap', 'required' => true];
        }
    ...
    private function cekMaterai(array $data): array
    {
        ...
        if ($total < 5000000) {
            return ['status' => 'info', 'message' => 'Tidak perlu materai (< Rp 5 juta)', 'required' => false];
        }
        ...
        return [
            ...
            'message' => $adaMaterai ? 'Materai terdeteksi' : 'Pastikan ada materai (transaksi > Rp 5 juta)',
            'required' => true,
        ];
    }
